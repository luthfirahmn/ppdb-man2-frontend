$("#cari_peserta").on('click', function(){
    ...
    var url = BASE_URL +  'undur_diri/get_peserta_undur_diri' + '/' + nisn;
    $.ajax({
        url: url,
        type: "post",
        dataType: "json",
        success: function(result) {
            if(result.error == false){
                ...
                html +='<tbody>';
                html +='<tr>';
                html +='<td>' + '<button type="button" onclick="action_undur_diri(\''+result.data.nisn +'\',70)"  class="btn btn-danger btn-sm" >Undurkan Diri</button>' + '</td>';
                ...
                swal.fire({
                    title: "Warning",
                    text: "Data Peserta Tidak Ditemukan atau peserta telah mengundurkan diri ",
                    type: "error"
                })
            }
...
function action_undur_diri(nisn,status)
{
    var url = BASE_URL + funct + '/aksi_undur_diri';
