<!DOCTYPE html>
<html>

<head>
    <title>Jadwal Tes BTQ</title>
    <style type="text/css">
        table {
            border-style: double;
            border-width: 3px;
            border-color: white;
        }

        table tr .text2 {
            text-align: right;
            font-size: 13px;
        }

        table tr .text {
            text-align: center;
            font-size: 13px;
        }

        table tr td {
            font-size: 13px;
        }
    </style>
</head>

<body>
    <center>
        <table style="width: 700px;">
            <tr>
                <td width="50"><img src="data:image/png;base64," width="90" height="90"></td>
                <td>
                    <center>
                        <!-- <font size="4">LEMBAGA PERATIKUM 2019</font><br> -->
                        <font size="4"><b>KEMENTRIAN AGAMA REPUBLIK INDONESIA</b></font><br>
                        <font size="3"><b>KANTOR KEMENTRIAN AGAMA KOTA BANDUNG</b></font><br>
                        <font size="3"><b>MADRASAH ALIYAH NEGERI 2</b></font><br>
                        <font size="2">Jalan Cipadung No. 57 Kec. CibiruTelp. ( 022 ) 63722262 Kota Bandung
                        </font><br>
                        <!-- <font size="2"><i>Jln Cut Nya'Dien No. 02 Kode Pos : 68173 Telp./Fax (0331)758005 Tempurejo
                                Jember Jawa Timur</i></font> -->
                    </center>
                </td>
            </tr>
            <tr>
                <td colspan="2">
                    <hr>
                </td>
            </tr>
            <table style="width: 700px;">
                <tr>
                    <td>
                        <center>KARTU PESERTA UJIAN TES BTQ<br>PENERIMAAN PESERTA DIDIK BARU TAHUN 2025/2026</center>
                    </td>
                </tr>
            </table>
        </table>

        <br>
        <br>
        <br>

        <table style="width: 700px;">
            <tr class="text2">
                <td>NAMA</td>
                <td width="572">: <?= $all_data->nama_lengkap ?></td>
            </tr>
            <tr>
                <td>NISN</td>
                <td width="564">: <?= $all_data->nisn ?></td>
            </tr>
            <tr>
                <td width="120">ASAL SEKOLAH</td>
                <td>: <?= $all_data->asal_sekolah ?></td>
            </tr>
        </table>
        <br>

        <br>
        <table width="625">
            <tr>
                <td>
                    <font size="2"><b>JADWAL UJIAN TES BTQ</b></font>
                </td>
            </tr>
        </table>
        <br>
        </table>
        <table>
            <tr class="text2">
                <td>Hari/Tanggal</td>
                <td width="541">: <b><?= date('d F Y', strtotime($all_data->tanggal)) ?></b></td>
            </tr>
            <tr>
                <td>Sesi/Waktu</td>
                <td width="525">: <b><?= $all_data->waktu ?></b></td>
            </tr>
        </table>
        <br>
        <style>
            .frame {
                width: 90px;
                /* 3 cm dikonversi ke piksel (1 cm ≈ 30px) */
                height: 120px;
                /* 4 cm dikonversi ke piksel */
                border: 1px solid black;
                /* Garis tepi frame */
                display: flex;
                align-items: center;
                justify-content: center;
                background-color: white;
                /* Latar belakang frame */
                text-align: center;
                font-size: 12px;
                font-weight: bold;
            }
        </style>
        <table style="width: 700px;">
            <tr>
                <td width="430"><br><br><br><br></td>
                <td align="right"><br>
                    <div class="frame">
                        <p>Pas foto 3x4</p>
                    </div>
                </td>
            </tr>
        </table>
        <!-- <table style="width: 700px;">
            <tr>
                <td width="330"><br><br><br><br></td>
                <td class="text" align="center">Bandung, 15 Mei 2024 <br>Ketua PPDB<br><br>TTD<br><br>Enjang Bakar,
                    S.pd</td>
            </tr>
        </table> -->
        <hr>
        <table width="625">
            <tr class="text2">
                <td><b>Catatan :</b></td>
            </tr>
            <tr>
                <td>• Bawa Alat Tulis</td>
            </tr>
            <tr>
                <td>• Membawa Kartu Peserta Tes <b>(DI PRINT)</b></td>
            </tr>
            <tr>
                <td>
                    • Dilarang membawa HP/alat komunikasi kedalam ruang ujian</td>
            </tr>
            <tr>
                <td>
                    • Datang tepat waktu</td>
            </tr>
            <tr>
                <td>
                    • Berpakaian rapih dan berseragan sekolah asal</td>

            </tr>



            </tr>
        </table>
    </center>
</body>

</html>
