<!DOCTYPE html>
<html>

<head>
    <title>Jadwal Tes Akademik</title>
    <style type="text/css">
        table {
            border-style: double;
            border-width: 3px;
            border-color: white;
        }

        table tr .text2 {
            text-align: right;
            font-size: 13px;
        }

        table tr .text {
            text-align: center;
            font-size: 13px;
        }

        table tr td {
            font-size: 13px;
        }
    </style>
</head>

<body>
    <center>
        <table style="width: 700px;">
            <tr>
                <td width="50"><img src="data:image/png;base64," width="90" height="90"></td>
                <td>
                    <center>
                        <!-- <font size="4">LEMBAGA PERATIKUM 2019</font><br> -->
                        <font size="4"><b>KEMENTRIAN AGAMA REPUBLIK INDONESIA</b></font><br>
                        <font size="3"><b>KANTOR KEMENTRIAN AGAMA KOTA BANDUNG</b></font><br>
                        <font size="3"><b>MADRASAH ALIYAH NEGERI 2</b></font><br>
                        <font size="2">Jalan Cipadung No. 57 Kec. CibiruTelp. ( 022 ) 63722262 Kota Bandung
                        </font><br>
                        <!-- <font size="2"><i>Jln Cut Nya'Dien No. 02 Kode Pos : 68173 Telp./Fax (0331)758005 Tempurejo
                                Jember Jawa Timur</i></font> -->
                    </center>
                </td>
            </tr>
            <tr>
                <td colspan="2">
                    <hr>
                </td>
            </tr>
            <table style="width: 700px;">
                <tr>
                    <td>
                        <center>KARTU PESERTA UJIAN TES AKADEMIK<br>PENERIMAAN PESERTA DIDIK BARU TAHUN 2023</center>
                    </td>
                </tr>
            </table>
        </table>
        <table style="width: 700px;">
            <tr class="text2">
                <td>NAMA</td>
                <td width="572">: <?= $all_data->nama_lengkap ?></td>
            </tr>
            <tr>
                <td>NISN</td>
                <td width="564">: <?= $all_data->nisn ?></td>
            </tr>
            <tr>
                <td width="120">ASAL SEKOLAH</td>
                <td>: <?= $all_data->asal_sekolah ?></td>
            </tr>
        </table>
        <br>
        <table width="625">
            <tr>
                <td>
                    <font size="2">JADWAL UJIAN TES AKADEMIK</font>
                </td>
            </tr>
        </table>
        <br>
        </table>
        <table>
            <tr class="text2">
                <td>Hari/Tanggal</td>
                <td width="541">: <b><?= $tanggal ?></b></td>
            </tr>
            <tr>
                <td>Sesi/Waktu</td>
                <td width="525">: <?= $all_data->jam ?></td>
            </tr>
            <tr>
                <td>Tempat</td>
                <td width="525">: <?= $all_data->ruangan ?></td>
            </tr>
        </table>
        <br>
        <table style="width: 700px;">
            <tr>
                <td width="330"><br><br><br><br></td>
                <td class="text" align="center">Bandung, 15 Mei 2024 <br>Ketua PPDB<br><br>TTD<br><br>Enjang Bakar,
                    S.pd</td>
            </tr>
        </table>
        <table width="625">
            <tr class="text2">
                <td><b>Catatan :</b></td>
            </tr>
            <tr>
                <td>• Materi Tes : 1. Literasi Membaca, 2. Literasi Sain 3. Literasi Sosbud, 4. Literasi Numerasi 5.
                    Literasi PAI </td>
            </tr>
            <tr>
                <td>• Bawa Alat Tulis</td>
            </tr>
            <tr>
                <td>• Membawa Kartu Peserta Tes <b>(DI PRINT)</b></td>
            </tr>
            <tr>
                <td>
                    • Dilarang membawa HP/alat komunikasi kedalam ruang ujian</td>
            </tr>
            <tr>
                <td>
                    • Datang tepat waktu</td>
            </tr>
            <tr>
                <td>
                    • Berpakaian rapih dan berseragan sekolah asal</td>

            </tr>



            </tr>
        </table>
    </center>
</body>

</html>
