...
class Template
{
	...
	public function build($view, $data = array(), $return = FALSE)
	{
		...
		$template['script']	= $this->_script;
		$template['style']	= $this->_style;
	...
	public function script($data)
	{
		if ($data['js']) {
			$array = array();
			foreach ($data['js'] as $v) :
				$array[] = '<script src="' . $v . '"></script>';
			endforeach;
		}
	...
	public function style($data)
	{
		if ($data['css']) {
			$array = array();
			foreach ($data['css'] as $v) :
				$array[] = '<link rel="stylesheet" href="' . $v . '">';
			endforeach;
		}
		$this->_style = $array;
		return $this;
