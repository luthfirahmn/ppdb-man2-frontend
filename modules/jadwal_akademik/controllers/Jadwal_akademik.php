...
class Jadwal_akademik extends MY_Controller
{
    ...
    public function __construct()
    {
        ...
        $this->model = $this->Jadwal_akademikModel;
    }
    ...
    public function index()
    {
        ...
        $this->template->title('Jadwal AKADEMIK List');
        $this->setTitlePage('Jadwal Akademik');
    ...
    public function get_list()
    {
        ...
        foreach ($all_data as $key => $val) {
            ...
            if ($val->sesi == 1) {
                ...
                $sesi = 'Error! ada sesi tidak ditemukan';
            }

            $no++;
            $row = array();
            // $row[] = buttons("edit", "location.href='" . base_url("/Jadwal_akademik/form_edit/" . $val->id . "") . "'") . ' ' . buttons("delete", "myDelete($val->id, $url_delete)");
            // $row[] = '';
            $row[] = $no;
            $row[] = $val->nisn;
            $row[] = $val->no_wa;
    ...
    public function export_jadwal($filter = '')
    {
        ...
        foreach ($user as $data) {
            $tanggal = $data->tanggal . ' JUNI 2022';
            ...
            $spreadsheet->setActiveSheetIndex(0)
                ...
                ->setCellValue('E' . $i, $sesi)
                ->setCellValue('F' . $i, $tanggal)
        ...
        // Rename worksheet
        $spreadsheet->getActiveSheet()->setTitle('JADWAL AKADEMIK ' . $filter);
        ...
        header('Content-Disposition: attachment;filename="JADWAL AKADEMIK.xlsx"');
    ...
    public function export_jadwals()
    {
        ...
        $excel->getProperties()->setCreator('MAN 2 Bandung')
            ...
            ->setTitle("JADWAL AKADEMIK")
            ->setSubject("JADWAL AKADEMIK")
            ->setDescription("JADWAL AKADEMIK")
            ->setKeywords("JADWAL AKADEMIK");
        ...
        $excel->getActiveSheet(0)->setTitle("JADWAL AKADEMIK");
        $excel->setActiveSheetIndex(0);
        // Proses file excel
        header('Content-Type: application/vnd.openxmlformats-officedocument.spreadsheetml.sheet');
        header('Content-Disposition: attachment; filename="JADWAL AKADEMIK.xlsx"'); // Set nama file excel nya
        header('Cache-Control: max-age=0');
