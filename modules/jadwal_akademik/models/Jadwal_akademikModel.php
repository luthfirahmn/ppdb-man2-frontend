...
class Jadwal_akademikModel extends MY_Model
{

    public function __construct()
    {
        parent::__construct();
        $this->table = 'ms_jadwal_akademik';
    }
    public function table_list($filter = '')
    {
        ...
        $this->db->join('ms_siswa siswa', 'siswa.nisn = table.nisn', 'left');
        $this->db->where("(table.nisn LIKE '%" . $_REQUEST['search']['value'] . "%' or
                            no_wa LIKE '%" . $_REQUEST['search']['value'] . "%' or
                            nama_lengkap LIKE '%" . $_REQUEST['search']['value'] . "%'
                            )");
        if (!empty($filter)) {
            $filter = date('d', strtotime($filter));
            $this->db->where('tanggal', $filter);
        }

        $this->db->order_by("tanggal,sesi,ruangan", "ASC");
    ...
    public function table_count_list($filter = '')
    {
    ...
    public function table_record_filter($filter = '')
    {
    ...
    public function get_all_data($param)
    {
        ...
        $this->db->join('ms_siswa siswa', 'siswa.nisn = table.nisn', 'left');
        if (!empty($param)) {
            $param = date('d', strtotime($param));
            $this->db->where('tanggal', $param);
        }
        return $this->db->get()->result();
    }
