...
class Peserta_ketm extends MY_Controller
{
    ...
    public function get_list()
    {
        ...
        $url_berkas = "peserta_ketm/download_berkas/";

        $url_delete = "'" . base_url("menu/delete") . "'";
        $data = array();
        $no = $_POST['start'];
        foreach ($all_data as $key => $val) {
            $query = $this->db->query("SELECT * FROM ms_siswa_berkas WHERE id_siswa = '{$val->id}' ");
            $res = $query->row();
            $test = '';
            if (empty($val->status_berkas)) {
                $test .= 'Berkas Belum Tersedia';
            } else {
                ...
                <a class="dropdown-item" href="' . BASE_URL . $url_berkas . $res->berkas_keterangan_lulus . '">Berkas Keterangan Lulus</a>
                
                </div>
                ';
                // $test .=
                //     '<select class="form-control">';
                // $test .= '<option value="' . $res->berkas_keterangan_lulus . '"> Keterangan Lulus </option>';
                // $test .=  '</select>';
            }
            ...
            $row[] =
                ...
                '<a href="' . BASE_URL . 'peserta_ketm/download_berkas/' . $val->id . '" class="btn btn-primary btn-sm" style="width:40px"><i class="fa fa-download"></i></a>';
            // $row[] = $test;
            $row[] = $val->nisn;
            $row[] = $val->no_wa;
    ...
    //         // $path = ASSETS_ROOT . 'images/' . $filename;
    //         $path = SISWA_BERKAS . $filename;
    //         // pre($path);  
    //         if (file_exists($path)) {
    //             echo "success";
