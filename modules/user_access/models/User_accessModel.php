...
class User_accessModel extends MY_Model
{

    public function __construct()
    {
        parent::__construct();
        $this->table        = 'ms_acl_group ';
        $this->table_menu   = 'ms_menu';
    }
    public function table_list()
    {
        ...
        $this->db->select("t0.*
                          ,ifnull(concat(t2.Menu,' > ',t1.Menu),t1.Menu) AS MenuName
                         ");
        $this->db->from($this->table . ' t0');
        $this->db->join($this->table_menu . ' t1', 't1.DID = t0.MenuID', 'left');
        $this->db->join($this->table_menu . ' t2', 't2.DID = t1.ParentID', 'left');
        $this->db->where("t1.MenuFile <> '' ");
        $this->db->where("(t1.Menu LIKE '%" . $_REQUEST['search']['value'] . "%'
                        or ifnull(concat(t2.Menu,' > ',t1.Menu),t1.Menu) LIKE '%" . $_REQUEST['search']['value'] . "%' 
                            )");

        if (empty($order)) {
            $this->db->order_by("t0.DID,t1.ParentID,t1.OrderNo", "DESC");
        } else {
            foreach ($order as $o) {
                $col = $o['column'];
                $dir = $o['dir'];
    ...
    public function updateAkses($post)
    {
        try {
            $this->tStart();
            $Status = $post['mode'] == 'unchecked' ? 1 : 0;
            $data = array(
                '' . $post['Act'] . ''  => $Status
            );
