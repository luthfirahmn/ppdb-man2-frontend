...
class Penilaian_akademikModel extends MY_Model
{

    public function __construct()
    {
        parent::__construct();
        $this->table = 'tr_penilaian';
    }
    public function table_list()
    {
        ...
        $this->db->select("table.nisn,
                            table.nama_lengkap,
                            (SELECT jalur FROM ms_jalur WHERE ms_jalur.id = table.id_jalur) as jalur,
                            (SELECT status_desc FROM ms_status WHERE status = table1.status) as status,
                            CONCAT('0',no_wa) as no_wa,
        ");
        $this->db->from($this->table . ' table1');
        $this->db->join("ms_siswa table", "table1.nisn = table.nisn");
