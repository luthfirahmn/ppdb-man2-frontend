...
class Penilaian_akademik extends MY_Controller
{
    ...
    public function __construct()
    {
        ...
        $this->model = $this->Penilaian_akademikModel;
    }
    ...
    public function index()
    {
        ...
        $this->template->title('Penialain Akademik List');
        $this->setTitlePage('Penilaian Peserta Akademik');
    ...
    public function get_list()
    {
        ...
        foreach ($all_data as $key => $val) {
            $no++;
            $row = array();
            // $row[] = buttons("edit", "location.href='" . base_url("/Penilaian_akademik/form_edit/" . $val->id . "") . "'") . ' ' . buttons("delete", "myDelete($val->id, $url_delete)");
            $row[] = $no;
            $row[] = $val->nisn;
            $row[] = $val->no_wa;
    ...
    public function download_template()
    {
        ...
        // Rename worksheet
        $spreadsheet->getActiveSheet()->setTitle('PENILAIAN AKADEMIK');
        ...
        header('Content-Disposition: attachment;filename="PENILAIAN AKADEMIK.xlsx"');
    ...
    public function upload_nilai()
    {
        ...
        if ($this->upload->do_upload('upload_nilai')) {
            ...
            foreach ($sheet as $row) {
                ...
                // $status = $row[1] == 'L' ? 8 : $row[1] == 'TL' ? 80  : $res_jalur->id_status;
                if ($row[1] == 'L') {
                    $status = 8;
                } else if ($row[1] == 'TL') {
                    $status = 80;
    ...
    public function penilaian()
    {
        ...
        $query = $this->db->query("SELECT * FROM tr_penilaian WHERE Type = 'AKADEMIK' ORDER BY id ASC");
        $result = $query->result();
